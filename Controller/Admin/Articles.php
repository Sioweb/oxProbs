...
class Articles extends AdminDetailsController
{
    ...
    public function render()
    {
        ...
        $aIncReports = [];
        $aIncFiles = $this->getConfigFiles('articles');
    ...
    private function _retrieveData()
    {
        ...
        switch ($cReportType) {
            ...
            case 'stockalert':
                if ($cReportType == 'nostock') {
                    ...
                    $txtStandard = Registry::getLang()->translateString("OXPROBS_STOCK_STANDARD");
                    $txtOffline = Registry::getLang()->translateString("OXPROBS_STOCK_OFFLINE");
                    $txtNotBuyable = Registry::getLang()->translateString("OXPROBS_STOCK_NOTBUYABLE");
            ...
            default:
                ...
                if (count($myConfig->getConfigParam("aOxProbsArticleIncludeFiles")) != 0) {
                    ...
                    $sIncPath = $this->jxGetModulePath() . "/articles/";
                    foreach ($aIncFiles as $sIncFile) {
        ...
        if (!empty($sSql1)) {
            ...
            if ($rs != false && $rs->count() > 0) {
                while (!$rs->EOF) {
                    array_push($aArticles, $rs->getFields());
                    $rs->fetchRow();
                }
            }
        }
